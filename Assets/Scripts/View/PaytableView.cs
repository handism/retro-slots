    ...
    public class PaytableView : MonoBehaviour
    {
        ...
        public void Populate(SymbolData[] symbols, PayoutTableData payoutData)
        {
            ...
            foreach (var sym in symbols)
            {
                ...
                if (texts.Length > 0)
                    texts[0].text = sym.payouts.Length > 0 ? sym.payouts[0].ToString("N0") : "-";
                if (texts.Length > 1)
                    texts[1].text = sym.payouts.Length > 1 ? sym.payouts[1].ToString("N0") : "-";
                if (texts.Length > 2)
                    texts[2].text = sym.payouts.Length > 2 ? sym.payouts[2].ToString("N0") : "-";
            }
